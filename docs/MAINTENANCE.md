...
- 默认模板 URL：`https://raw.githubusercontent.com/Spittingjiu/clash-generic-template/main/clash-template.json`

## 关键仓库
- 面板与后端：`Spittingjiu/sui-sub`
- 通用模板：`Spittingjiu/clash-generic-template`
- 白名单规则：`Spittingjiu/clash-custom-rules`

## 日常改动流程（建议）
1. 先改模板仓库（`clash-generic-template`）
2. 运行本地校验脚本（`scripts/validate-template.mjs`）
3. 再改 `sui-sub`（仅在需要回退模板同步时）
4. 发布后在面板执行一次订阅更新，检查日志

## 回滚
- 模板回滚：在 `clash-generic-template` 回退到上个 commit 并 push
